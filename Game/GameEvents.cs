...
class GameEvents
{
    ...
    public static void EmptyCellRevealed(int row, int col, int neighbouringMines) =>
        OnEmptyCellRevealed?.Invoke(row, col, neighbouringMines);

    public static void MineCellRevealed(int row, int col) => OnMineCellRevealed?.Invoke(row, col);

    public static void DifficultyChanged(Difficulty newDifficulty) =>
        OnDifficultyChanged?.Invoke(newDifficulty);

    public static void NewGame() => OnNewGame?.Invoke();

    public static void GridInitialized(int rows, int cols, Difficulty difficulty) =>
        OnGridInitialized?.Invoke(rows, cols, difficulty);

    public static void FlagSet(int row, int col, bool isFlagged) =>
        OnFlagSet?.Invoke(row, col, isFlagged);

    public static void FlagCounterUpdate(int newValue) => OnFlagCounterUpdate?.Invoke(newValue);

    public static void HighlightCells(List<(int, int)> cellsToHighlight) =>
        OnCellsHighlighted?.Invoke(cellsToHighlight);

    public static void GameWon() => OnGameWon?.Invoke();

    public static void UpdateActiveCells(List<(int, int)> activeCells) =>
        OnUpdateActiveCells?.Invoke(activeCells);
