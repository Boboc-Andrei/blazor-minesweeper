...
namespace MinesweeperDotNET.Solver
{
    public class MinesweeperSolver
    {
        ...
        private void GenerateGroups()
        {
            ...
            foreach (var cell in grid.ActiveCells)
            {
                if (!cell.IsRevealed)
                    continue;
                List<Cell> unrevealedNeighbours = grid.GetUnrevealedNeighbours(
                    cell,
        ...
        private void ScanForMultipleGroupHints()
        {
            ...
            {
                if (!grid.ActiveCells.Contains(currentGroup.Owner))
                    continue;
                if (currentGroup.IsMineGroup || currentGroup.IsRevealable)
                    continue;
                foreach (CellGroup otherGroup in groups)
                {
                    if (otherGroup.IsMineGroup || otherGroup.IsRevealable)
                        continue;
                    if (!currentGroup.HasOverlapWith(otherGroup) || otherGroup == currentGroup)
                        continue;

                    CellGroup minesGroup,
                        revealGroup;
        ...
        public void OnUserToggledFlag(Cell cell)
        {
            ...
            if (cell.IsFlagged)
            {
                if (flaggableCells.Contains(cell))
                    return;
