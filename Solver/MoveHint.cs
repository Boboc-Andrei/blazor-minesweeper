...
public class FlagsSatisfiedHint : MoveHint
{
    ...
    public FlagsSatisfiedHint(CellGroup cellGroup, MinesweeperGrid grid, MinesweeperSolver solver)
        : base(cellGroup, grid, solver) { }

    public override List<Cell> GetAffectedCells()
    {
        List<Cell> affectedCells = Grid.GetUnrevealedNeighbours(
            CellGroup.Owner,
            includeFlagged: true
        );
    ...
    {
        return Grid.GetUnrevealedNeighbours(CellGroup.Owner, includeFlagged: false).Count == 0
            || !CellGroup.Owner.HasAllMinesFlagged;
    }
...
public class FlagGroupHint : MoveHint
{
    ...
    public FlagGroupHint(CellGroup cellGroup, MinesweeperGrid grid, MinesweeperSolver solver)
        : base(cellGroup, grid, solver) { }
...
public class WrongFlagHint : MoveHint
{
    ...
    public WrongFlagHint(CellGroup cellGroup, MinesweeperGrid grid, MinesweeperSolver solver)
        : base(cellGroup, grid, solver) { }
    ...
    public override bool IsObsolete()
    {
        return !CellGroup.Owner.IsFlagged
            || (CellGroup.Owner.IsFlagged && Solver.flaggableCells.Contains(CellGroup.Owner));
    }
...
{
    public RevealCellsHint(CellGroup cellGroup, MinesweeperGrid grid, MinesweeperSolver solver)
        : base(cellGroup, grid, solver) { }
